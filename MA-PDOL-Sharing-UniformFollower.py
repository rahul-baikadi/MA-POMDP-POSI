"""
MA-PDOL-Sharing-UniformFollower: Shared Counts + Uniform Follower Selection
============================================================================

Based on MA-PDOL-Sharing with ONE modification:
  - Follower selection uses UNIFORM random sampling from the augmented
    set (excluding the leader), replacing the fresh_pool rotation in
    Eq. (12).  No memory of recently queried coordinates is kept.
...
This tests whether the rotation-based follower coverage mechanism
matters, or whether simple uniform sampling is sufficient.

Usage:
    python MA-PDOL-Sharing-UniformFollower.py
"""
...
###########################################################################
# MA-PDOL-Sharing-UniformFollower Algorithm
###########################################################################

class MAPDOLSharingUniformFollower:
    ...
    def _form_query_set(self, ag: AgentState) -> np.ndarray:
        """Uniform follower selection from augmented set (no rotation)."""
        need = self.dt - 1
        B = list(ag.non_leader)

        if not B:
            # Fallback: sample from all coordinates except leader
            pool = [j for j in range(self.d) if j != ag.leader]
            n_sample = min(need, len(pool))
            F = list(np.random.choice(pool, n_sample, replace=False))
        elif len(B) <= need:
            # Augmented set too small — take all of it
            F = list(B)
        else:
            # CHANGED: simple uniform random, no fresh_pool tracking
            ix = np.random.choice(len(B), need, replace=False)
            F = [B[i] for i in ix]
    ...
    def run(self, verbose: bool = False) -> dict:
        """
        Run MA-PDOL-Sharing with uniform follower selection.
        Only difference: followers are sampled uniformly from the
        augmented set instead of using fresh_pool rotation.
        """
...
def main():
    ...
    print("=" * 65)
    print("MA-PDOL-Sharing-UniformFollower — Per-Agent Regret")
    print("=" * 65)
    ...
    for N_ in N_values:
        np.random.seed(0)
        env = base_env.clone()
        algo = MAPDOLSharingUniformFollower(
            env=env, N=N_, d_tilde=d_tilde, K=K)
        label = f"UniformFollower N={N_}"
    ...
    plot_per_agent_regret(
        results,
        f'MA-PDOL_uniformfollower_regret_{datetime.now().strftime("%m%d_%H%M")}.png',
        f'MA-PDOL-Sharing-UniformFollower — Per-Agent Regret vs Episode\n'
